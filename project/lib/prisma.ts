// 根據環境使用真實或模擬的 PrismaClient
import { PrismaClient } from '@prisma/client';
import { mockUsers, mockCategories, mockProducts, mockWarehouses, mockInventoryItems } from './mock-data';

// 檢查是否在 WebContainer 環境中運行
const isWebContainer = process.env.IS_WEBCONTAINER === 'true';

// 定義導出的 prisma 變量
let prisma: any;

// 如果在 WebContainer 中，使用模擬客戶端
if (isWebContainer) {
  console.log('Using Mock PrismaClient for WebContainer environment');
  
  // 這裡保留原來的 MockPrismaClient 實現
  // 定義類型
  ...
  // 在非 WebContainer 環境中使用真實的 PrismaClient
  console.log('Using real PrismaClient for production environment');
